...
func Run(port int) {
	...
	dataRepository := data_repository.NewDataGORMRepository(gormdb)

	// Create all of our services.
	userService := user.NewService(userRepository)
	dataService := data.NewService(dataRepository)

	api := app.Group("/api")

	// Prepare our endpoints for the API.
	handler.NewUserHandler(api.Group("/v1/users"), userService)
	handler.NewDataHandler(api.Group("v1/datas"), dataService)
